...
def start_dolphin_emu(instance, selected_game):
    ...
    if instance > 1:
        print("oh hell naw, dont launch multiple dolphins")
    return instance

def change_game_or_quit(shutdown):
    if shutdown == 1:
        for z in range(0,25):
            call("pkill dolphin-emu", shell = True)
    if shutdown == 4:
...
while 1:
    ...
    for event in events:
        ...
        if "BTN_BASE3" in menu_info:
            instance = 0
            shutdown = shutdown + 1
            change_game_or_quit(shutdown)
